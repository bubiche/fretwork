 ...
 * the target voice immediately AFTER `at`, shifting existing beats right (insert-and-shift —
 * nothing overwritten). All beats land in `at`'s single bar/voice, so the bar may
 * overflow its time signature — alphaTab renders an overfull bar as-is; there is **no auto-rebar**
 * (out of scope). `relayout: 'voice'` so `finish()` reindexes/re-chains and
 * re-resolves any linked-note pointers the pasted beats carry (verified: a range whose tie/slide/HOPO
 * partner sat outside the copied region splices and finishes without dangling).
 *
